 ...
 */
  ...
  async setValue(element, text) {
    ...
    await this.typeWithRandomDelay(element, text);
  }

  async typeWithRandomDelay(element, text, min = 100, max = 250) {
  ...
  /**
   * Waits until more than one element matching the selector is displayed on the page.
   *
   * @param {string} selector - The selector used to find elements (CSS or XPath).
   * @param {number} [timeout=30000] - Maximum wait time in milliseconds before timing out.
   * @returns {Promise<void>} Resolves when more than one element is found and displayed.
   * @throws {Error} Throws an error if the condition is not met within the timeout.
   */
  async waitUntilMoreThanOneElementDisplayed(selector, timeout = 30000) {
    await browser.waitUntil(async () => {
      ...
      timeout,
      timeoutMsg: `No visible elements found for selector: ${selector}`,
    });
  }
}